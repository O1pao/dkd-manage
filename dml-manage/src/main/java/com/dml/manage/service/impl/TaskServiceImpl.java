...
import com.dml.manage.domain.dto.TaskDetailDto;
import com.dml.manage.domain.dto.TaskDto;
...
import com.dml.manage.mapper.TaskMapper;
import com.dml.manage.domain.Task;
import com.dml.manage.service.ITaskService;
...
public class TaskServiceImpl implements ITaskService
{
    ...
    public int insertTaskDto(TaskDto taskDto) {
        ...
        task.setTaskStatus(DelContents.TASK_STATUS_CREATE); // 工单状态设置为创建
        ...
        // 判断是否为补货工单
        if (taskDto.getProductTypeId().equals(DelContents.TASK_TYPE_SUPPLY)) {
            // 保存工单详情
    ...
    public int cancelTask(Task task) {
        ...
        // 判断工单状态是否是已取消
        if (Objects.equals(taskDb.getTaskStatus(), DelContents.TASK_STATUS_CANCEL))
            throw new SecurityException("工单已经是取消的了");
        // 判断工单状态是否是已完成
        if (Objects.equals(taskDb.getTaskStatus(), DelContents.TASK_STATUS_FINISH))
            throw new SecurityException("已完成的工单不允许取消");
        // 更新字段
        task.setTaskStatus(DelContents.TASK_STATUS_CANCEL);
        task.setUpdateTime(DateUtils.getNowDate());
        // 更新工单状态为取消
        return taskMapper.updateTask(task);
    }
    ...
    private String generateTaskCode(){
        ...
        // 根据日期生成redis的键
        String key = "dml.task.code." + dateStr;
        // 判断key是否存在
    ...
    private void hasTask(TaskDto taskDto) {
        ...
        taskParam.setTaskStatus(DelContents.TASK_STATUS_PROGRESS);
    ...
    private void checkCreateTask(Long vmStatus, Long productTypeId){
        // 如果是投放工单，设备在运行中，抛出异常
        if (productTypeId == DelContents.TASK_TYPE_DEPLOY && vmStatus == DelContents.VM_STATUS_RUNNING) {
            throw new SecurityException("设备正在运行中，无法进行投放操作");
        }
        // 如果是维修工单，设备不在运行中，抛出异常
        if (productTypeId == DelContents.TASK_TYPE_REPAIR && vmStatus != DelContents.VM_STATUS_RUNNING) {
            throw new SecurityException("设备不在运行中，无法进行维修操作");
        }
        // 如果是补货工单，设备不在运行中，抛出异常
        if (productTypeId == DelContents.TASK_TYPE_SUPPLY && vmStatus != DelContents.VM_STATUS_RUNNING) {
            throw new SecurityException("设备不在运行中，无法进行补货操作");
        }
        // 如果是撤机工单，设备不在运行中，抛出异常
        if (productTypeId == DelContents.TASK_TYPE_REVOKE && vmStatus != DelContents.VM_STATUS_RUNNING) {
